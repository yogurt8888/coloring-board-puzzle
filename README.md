...
纯 Python，只依赖 Pillow，Windows / macOS / Linux 通用。已经内置第 1~17 关的答案，
重复的关卡和直接报关卡号都是秒回。
...
- 本地已经有的关卡（1~17 关这种）**完全不联网**，仍是 0.05 秒；
...
```bash
# 1) 从截图求解（最常用）
python scripts/pipeline.py --img 截图.png --level 17 --target Y --budget 5 \
...
│   └── levels/                ← 17 关成品图 + levels.json 索引
├── examples/                  ← 17 关棋盘存档（board_level1..17.txt）
├── tools/
│   ├── selfcheck.py           ← 自检：环境 / 字体 / 索引 / 三道校验
│   ├── build_level_index.py   ← 从棋盘现算并重建关卡索引
│   ├── step_detail.py         ← 逐关打印「每步并了哪几片」（写关卡详解时核对用）
