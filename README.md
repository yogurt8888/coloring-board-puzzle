...
纯 Python，只依赖 Pillow，Windows / macOS / Linux 通用。已经内置第 1~15 关的答案，
重复的关卡和直接报关卡号都是秒回。
...
- 本地已经有的关卡（1~15 关这种）**完全不联网**，仍是 0.05 秒；
...
```bash
# 1) 从截图求解（最常用）
python scripts/pipeline.py --img 截图.png --level 15 --target B --budget 5 \
...
│   └── levels/                ← 15 关成品图 + levels.json 索引
├── examples/                  ← 15 关棋盘存档（board_level1..15.txt）
├── tools/
